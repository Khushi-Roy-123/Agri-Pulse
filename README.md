...
```
*(Note: This downloads TensorFlow. If you see a warning about GPU support on Windows, it is completely normal and the code will still run perfectly on your CPU).*
...
2. **Image Data:** Download the [PlantVillage Dataset](https://www.kaggle.com/datasets/abdallahal/plantvillage-dataset). Extract it so the `PlantVillage` directory is placed inside the `data/` folder. The Diagnostic Engine will actively open random JPGs from these sub-folders to run real-time inference checks!
